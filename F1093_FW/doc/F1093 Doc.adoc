...
=== Setup
The following tools and libraries are used to build this firmware:
...
==== Arduino Board Setup
The board setup (in the Tools tab) is setup as follows:
...
=== Required File
To build this firmware, a file titled `wifiDefault.h` must be created in the firmware's root path. The file must contain the following:
...
|ledColor
a|[str], The new LED color mode, either:

- 'manual'
- 'rainbow'
- 'timeHue'
|
|Sets the LED color mode

|ledBlinkMode
a|[str], The new LED blink mode, either:

- 'off'
- 'sec'
|
|Sets the LED blink mode
